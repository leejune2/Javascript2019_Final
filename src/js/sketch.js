...
//only when that is complete process article description text to a string
 ...
 .then(function(response){
 	...
 	.then(function(resp){
        ...
        // put article descriptions in allText
		getImages(allText);
	});
}

//take the string and make it an array, then pick random words to make sure they are not stop words
...
// parse results, find the parts that we want and create a DOM element for each image
		...
		var attr = document.createAttribute("id");
 		attr.value = "imageAndTextContainer";

// create div with img tag and insert text with associated title from randomList
		var imageAndText = document.createElement("div");
		imageAndText.innerHTML = "<img src="+image[0]["largeImageURL"]+" title="+randomList[i]+">" + 
		"<div>" + randomList[i] + "</div>"; 
		imageAndText.setAttribute('id', "imageAndTextContainer");

		document.body.appendChild(imageAndText);
